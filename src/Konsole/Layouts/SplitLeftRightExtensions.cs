    ...
    public static class SplitLeftRightExtensions
    {
        ...
        internal static (IConsole left, IConsole right) _SplitLeftRight(IConsole c, string leftTitle, string rightTitle, LineThickNess? thickness, BorderCollapse border, ConsoleColor foreground, ConsoleColor background)
        {
            lock (Window._locker)
            {
                ...
                var leftWin = new Window(c, new WindowSettings { SX = 1, SY = 1, Width = leftWidth, Height = h - 2, Theme = theme, HasBorder = false, Title = leftTitle });
                var rightWin = new Window(c, new WindowSettings { SX = leftWidth + 2, SY = 1, Width = rightWidth, Height = h - 2, Theme = theme, HasBorder = false, Title = rightTitle });
